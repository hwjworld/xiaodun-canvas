  ...
  class NotificationPreferences
    ...
    constructor: (@options) ->
      ...
      @buttonData = [
        code: 'immediately'
        icon: 'icon-check'
        text: I18n.t('frequencies.immediately', '尽快')
        title: I18n.t('frequencies.title.right_away', '立即通知我')
      ,
        code: 'daily'
        icon: 'icon-clock'
        text: I18n.t('frequencies.daily', '每天')
        title: I18n.t('frequencies.title.daily', '发送每日摘要')
      ,
        code: 'weekly'
        icon: 'icon-calendar-month'
        text: I18n.t('frequencies.weekly', '每周')
        title: I18n.t('frequencies.title.weekly', '发送每周摘要')
      ,
        code: 'never'
        icon: 'icon-x'
        text: I18n.t('frequencies.never', '从不')
        title: I18n.t('frequencies.title.never', '不发送任何内容')
      ]
      ...
      for c in @channels
        c.name = switch c.type
          when 'email' then I18n.t('communication.email.display', '电子邮件地址')
          when 'sms' then I18n.t('communication.sms.display', '手机号码')
          when 'push' then I18n.t('communication.push.display', '推送通知')
    ...
    saveNewCellValue: ($cell, value) =>
      ...
      @$notificationSaveStatus.disableWhileLoading $.ajaxJSON(@updateUrl, 'PUT', data, null,
        # Error callback
        ((data) =>
          $.flashError(I18n.t('communication.errors.saving_preferences_failed', '啊噢! 有些内容已损坏。请重试'))
        )
      ), @spinOpts
